(() => {
 ...
 ╚══██╔══╝██╔══██╗████╗  ██║
  ...
  function drawNoise(intensity01) {
    ...
    const alphaBase = 0.06 + intensity01 * 0.22;
  ...
  function mutateLine(base) {
    ...
    if (p <= 0) return base;
    let s = base;
    if (chance(0.15 * p)) s = s.replace(/[aeiou]/gi, (m) => (chance(0.4) ? "" : m));
    if (chance(0.20 * p)) s = s.replace(/[A-Z]/g, (m) => (chance(0.1) ? String.fromCharCode(m.charCodeAt(0) + 1) : m));
    if (chance(0.10 * p)) s = s.split("").map(ch => (chance(0.05) ? "█" : ch)).join("");
    return s;
  }
  ...
  function maybeVhsFault() {
    if (!chance(0.006 + visualMadness * 0.035)) return;
    fuzzEl.style.top = `${rand(3, 78)}%`;
    fuzzEl.style.opacity = (0.20 + visualMadness * 0.55).toFixed(2);
  ...
  function generateLine() {
    ...
    } else if (comprehension < 0.90) {
      base = chance(0.50) ? pick(POOL_B) : pick(POOL_C);
    } else {
      const r = Math.random();
      if (r < 0.40) base = pick(POOL_C);
  ...
  window.addEventListener("resize", fitNoiseCanvas);
  fitNoiseCanvas();
  intro();
})();
